"""
Scalability-enhanced MILP solver for the data-centre scheduling model, with the following enhancements.

  A. Tighter wear big-M (stronger relaxation).
     The conditional wear-reset linearisation (c24b/c24c) originally used ONE
     global M_psi = max(psi_0) + nK*max(C_cpu) for every server. Here each
     server gets its own M_psi[j] = psi_0[j] + nK*C_cpu[j]: still a valid upper
     bound on the accumulation w[j,k] (start wear plus at most full CPU load in
     every slot before a reset), but far tighter for fresh / low-capacity
     servers, which sharpens the LP bound the branch-and-bound relies on.

  B. Symmetry breaking + branching priority.
     Sets Gurobi Symmetry=2 (fleets contain many interchangeable servers), and
     raises the branch priority of the structural on/off (y) and preventive-
     maintenance (v) binaries above the assignment binaries (X): fixing y/v
     collapses most of the downstream feasibility, so branching on them first
     shrinks the tree.

  C. MIP warm start (starting incumbent).
     A fast greedy constructor places each job's replicas on distinct eligible
     servers at the earliest valid start with residual CPU+mem capacity, and
     feeds that as a PARTIAL MIP start. Gurobi then begins from an incumbent
     instead of searching for a first feasible point; an infeasible hint is
     simply discarded by the solver at no cost.

  D. Scale-friendly Gurobi parameters.
     On top of the Presolve/MIPFocus/Cuts/Heuristics tuning inherited from
     optimizer.py, adds NodefileStart=0.5 (spill the search tree to disk before
     large instances exhaust RAM) and NoRelHeurTime (a dedicated no-relaxation
     heuristic, scaled to the time budget, that searches for a first incumbent
     on hard instances where LP-based heuristics and a partial MIP start often
     fail to complete).
...
_DEFAULT_OUTPUT_DIR = _REPO_ROOT / "outputs" / "experimental_results_v0.2"
...
def solve_datacenter_model(
    ...
    # [Enhancement B/D] Aggressive symmetry detection (many servers are
    # identical), a memory-safe tree spill so large instances do not run the
    # solver out of RAM, and a dedicated no-relaxation heuristic that hunts for
    # a first incumbent on hard instances (where the LP-based heuristics and a
    # partial MIP start often fail to complete). Scaled to the time budget.
    mdl.setParam("Symmetry", 2)
    mdl.setParam("NodefileStart", 0.5)
    mdl.setParam("NoRelHeurTime", max(20.0, min(0.1 * time_limit, 300.0)))
    if not verbose:
    ...
    # [Enhancement A] Per-server wear big-M instead of one global scalar.
    # psi_0[j] + nK*C_cpu[j] is still a valid upper bound on w[j,k] (starting
    # wear plus at most full CPU load in every slot before a reset), but it is
    # far tighter than max(psi_0) + nK*max(C_cpu) for fresh / low-capacity
    # servers, which strengthens the LP relaxation of c24b/c24c below.
    M_psi = {j: psi_0[j] + nK * C_cpu[j] for j in J}
    psi = {(j, k): mdl.addVar(lb=0.0, ub=M_psi[j], name=f"psi_{j}_{k}")
           for j in J for k in K}
    ...
    w = {(j, k): mdl.addVar(lb=0.0, ub=M_psi[j], name=f"w_{j}_{k}")
         for j in J for k in K}
    ...
    # [Enhancement B] Branch on the structural decisions first. Fixing the
    # on/off (y) and preventive-maintenance (v) binaries determines most of the
    # downstream feasibility, so give them higher branch priority than the
    # assignment binaries (X), which keep the default priority of 0.
    for _var in y.values():
        _var.BranchPriority = 10
    for _var in v.values():
        _var.BranchPriority = 8
    ...
    for j in J:
        for k in K:
            ...
            mdl.addConstr(psi[j, k] <= w[j, k],
                          name=f"c24a_{j}_{k}")
            mdl.addConstr(psi[j, k] <= M_psi[j] * (1 - z[j, k]),
                          name=f"c24b_{j}_{k}")
            mdl.addConstr(psi[j, k] >= w[j, k] - M_psi[j] * z[j, k],
                          name=f"c24c_{j}_{k}")
    ...
    # -----------------------------
    # 4.6 Warm start  [Enhancement C]
    # -----------------------------
    # Greedy constructive MIP start. Place each job's q[i] replicas on distinct
    # eligible servers at the earliest valid start that has residual CPU AND
    # memory capacity across the job's whole run, tracking per-(server,slot)
    # residual capacity as we go. Two capacity budgets are respected so the hint
    # does not violate the packing constraints: the TOTAL server capacity C
    # (c11) and, for batch jobs, the smaller interactive-reserved capacity
    # (1-theta)*C (c8). Only the chosen X (=1) and the servers they switch on
    # (y=1) are hinted; every other variable is left UNDEFINED so Gurobi
    # completes the start itself. This is a partial MIP start: if it still
    # cannot be completed to a feasible incumbent Gurobi discards it for free,
    # so the greedy need not honour precedence / affinity / rack diversity /
    # thermal -- those are repaired (or the hint dropped) by the solver.
    res_tot = {rn: {(j, k): C_res[rn][j] for j in J for k in K} for rn in RES}
    res_batch = {
        rn: {(j, k): (1.0 - theta_res[rn][j]) * C_res[rn][j]
             for j in J for k in K}
        for rn in RES
    }
    y_on_hint = set()
    for i in sorted(I, key=lambda ii: (a[ii], -d[ii])):
        is_batch = i in I_B
        used_servers = set()
        for _rep in range(q[i]):
            chosen = None
            for k in valid_starts(i):
                span = range(k, k + d[i])
                for j in S[i]:
                    if j in used_servers or (i, j, k) not in X:
                        continue
                    fits = all(res_tot[rn][j, kk] >= r_res[rn][i]
                               for rn in RES for kk in span)
                    if is_batch:
                        fits = fits and all(res_batch[rn][j, kk] >= r_res[rn][i]
                                            for rn in RES for kk in span)
                    if fits:
                        chosen = (j, k, span)
                        break
                if chosen is not None:
                    break
            if chosen is None:
                continue  # leave this replica for Gurobi to place
            j, k, span = chosen
            X[i, j, k].Start = 1
            used_servers.add(j)
            for kk in span:
                for rn in RES:
                    res_tot[rn][j, kk] -= r_res[rn][i]
                    if is_batch:
                        res_batch[rn][j, kk] -= r_res[rn][i]
                y_on_hint.add((j, kk))
    for (j, k) in y_on_hint:
        y[j, k].Start = 1
...
def main() -> None:
    ...
    print("=== Scalability-enhanced solver (optimizer_v0.2.py) ===")
    print("    A: per-server wear big-M | B: symmetry + branch priority | "
          "C: greedy warm start | D: NodefileStart")
    ...
    if args.jobs_json_input and args.server_json_input:
        ...
        solver_stats_csv_path = output_json_path.parent / "optimizer_v0.2_stats_summary.csv"
    ...
    solver_stats_csv_path = output_dir / "optimizer_v0.2_stats_summary.csv"
